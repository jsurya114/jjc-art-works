...
export default function ManageGallery() {
  const [activeTab, setActiveTab] = useState('main'); // 'main', 'restorations', 'more'
  ...
  // Main Gallery specific metadata
  const [title, setTitle] = useState('');
  const [category, setCategory] = useState('Chapel Pews');

  const categories = ['All Work', 'Chapel Pews', 'Altar Furniture', 'Custom Woodwork', 'Church Seating', 'Pulpits'];
  ...
  const handleUpload = async (e) => {
    ...
    if (activeTab === 'main' && (!title || !category)) {
      alert("Please provide a title and category for the Main Gallery.");
      return;
    }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <div className="flex space-x-1 bg-slate-100 p-1 rounded-lg w-max">
        {['main', 'restorations', 'more'].map(tab => (
          <button
            key={tab}
